...
gas: 3379755
submitted: 2018-06-10 02:23:16.619769+00:00
---
```solidity
...
contract HexDecoder {
    function decode(string input) public pure returns(bytes) {
        ...
        for (uint i = 0; i < bytes(input).length; i++) {
            if (odd) {
                if (bytes(input)[i] >= 97) {
                    output[j] = byte(p + (uint(bytes(input)[i]) - 87));
                } else if (bytes(input)[i] >= 65) {
                    output[j] = byte(p + (uint(bytes(input)[i]) - 55));
                } else {
                    output[j] = byte(p + (uint(bytes(input)[i]) - 48));
                }
                odd = false;
                j++;
            } else {
