...
challenge: Unique
gas: 119241
submitted: 2018-06-27 21:35:24.755693+00:00
---
```solidity
...
contract Unique {
    ...
    // setting k=1 and having a larger bitfield (0x40 machine words: 2048 bytes, 2^21 bits)
    ...
    // We know there's at least one array element, so fall into the loop
        loop_start:
            ...
            // from 0 to 2,032 that is divisible by 32
            // i.e. memory indices for 32 byte words from 0 to 64.
            ...
            dup1 0x7e0 and 0x100 add    // stack state: i h s
    ...
    // ### RETURN
            ...
            // 0x40 - rest: array
            // We know our array data starts at 0x920, so store '0x20' 2 words beneath that
            0x20 0x8e0 mstore          // stack state: s
            // Get size (in bytes) of array.
            0x920 msize sub            // stack state: l s
            // Divide size by 0x20 to get number of elements, store at index 0x900
            0x20 dup2 div 0x900 mstore // stack state: l s
            // Add 0x40 to size (in bytes) to get total size of output datam then return
            0x40 add 0x8e0 return
    // ### SKIP ADD TO SET
            ...
            0x20 0x8e0 mstore          // stack state:
            0x920 msize sub            // stack state: l
            0x20 dup2 div 0x900 mstore // stack state: l
            0x40 add 0x8e0 return
