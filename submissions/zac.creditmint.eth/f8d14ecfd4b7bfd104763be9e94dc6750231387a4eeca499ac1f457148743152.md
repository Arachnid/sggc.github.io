...
challenge: BrainFuck
gas: 175618
submitted: 2018-06-18 18:56:48.519611+00:00
---
```solidity
/**
...
contract BrainFuck {
    ...
    function execute(bytes, bytes) external view returns(bytes) {
        assembly {
                ...
                increment_data_cell:
                    mstore8(data_pointer, add(mload(sub(data_pointer, 0x1f)), 0x01))
                    program_counter := add(program_counter, 0x20)
                    jump(mload(program_counter))
                ...
                conditional_jump_forward:
                    jumpi(skip_jump_forward, and(mload(sub(data_pointer, 0x1f)), 0xff))
                ...
                conditional_jump_backward:
                    jumpi(jump_backward, and(mload(sub(data_pointer, 0x1f)), 0xff))
                    program_counter := add(program_counter, 0x40)
                    jump(mload(program_counter))
