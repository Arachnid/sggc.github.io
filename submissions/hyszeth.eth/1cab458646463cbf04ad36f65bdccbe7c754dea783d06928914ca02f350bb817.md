...
challenge: Unique
gas: 434730
submitted: 2018-06-23 20:31:28.888929+00:00
---
```solidity
/**
        ...
        for(uint i = 0; i < inputLength; ++i) {
            ...
            if((currentCache=cache[quickHash]) > 0) {
                ...
                while(currentCache != currentCacheOffset) {
                    //miss++;
                    // Look for next spot
                    currentCache = cache[(quickHash *= currentCacheOffset^RAND_OFFSET) % HASH_TABLE_SIZE];
                    if(currentCache == 0) {
                        if(uniqueIndex != i) input[uniqueIndex] = current;
                        uniqueIndex++;
                        cache[quickHash % HASH_TABLE_SIZE] = currentCacheOffset;
                        break;
                    }
                }
            } else {
