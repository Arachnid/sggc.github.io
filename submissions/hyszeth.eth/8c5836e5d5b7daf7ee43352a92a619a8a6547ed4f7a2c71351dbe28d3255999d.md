...
challenge: Unique
gas: 330027
submitted: 2018-06-28 05:29:34.719183+00:00
---
```solidity
/**
...
contract Unique {
    ...
    returns(uint[]) {
        ...
        for(;i != inputLength;) {
            current = input[i];
            if((currentCacheOffset = current + RAND_OFFSET) == last) {
                ++i;
                continue;
            }
            if((currentCache=cache[(quickHash = current % HASH_TABLE_SIZE)]) == 0) {
                ...
                uniqueIndex++;
                cache[quickHash] = last = currentCacheOffset;
                continue;
            }
