...
challenge: Sort
gas: 951913
submitted: 2018-06-16 20:33:24.581597+00:00
---
```solidity
/**
...
contract Sort {
      ...
      returns(uint[])
    {
         ...
         pure
        {
            ...
            if(lo < j) {
                if(j - lo > 1) quickSort(input, lo, j);
                else if((iElem=input[lo]) > (jElem=input[j])) {
                    (input[lo], input[j]) = (jElem, iElem);
                }
            }
            j++;
            if(j < hi) {
                if(hi - j > 1) quickSort(input, j, hi);
                else if((iElem=input[j]) > (jElem=input[hi])) {
                    (input[j], input[hi]) = (jElem, iElem);
                }
            }
        }
