...
challenge: BrainFuck
gas: 2397793
submitted: 2018-05-29 07:46:04.129086+00:00
---
```solidity
/**
...
contract BrainFuck {
    ...
    function execute(bytes program, bytes input) public pure returns(bytes) {
        ...
        for (iidx = 0; iidx < ret.length; iidx++) {
            ret[iidx] = output[iidx];
        }
        return ret;
    }
    ...
    function clean(bytes program) internal pure returns(uint) {
        ...
        while (i < program.length) {
            token = program[i];
            if (       token == 0x3c || token == 0x3e
                    || (token > 0x2a && token < 0x2f)
                    || token == 0x5b || token == 0x5d) {
                program[j] = token;
                j++;
            }
            i++;
