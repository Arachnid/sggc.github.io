...
challenge: BrainFuck
gas: 3161942
submitted: 2018-05-25 18:16:57.632659+00:00
---
```solidity
/**
...
contract BrainFuck {
    ...
    function execute(bytes program, bytes input) public pure returns(bytes) {
      ...
      uint[1024] memory output;
      uint output_pointer = 0;
      uint input_pointer = 0;

      uint[] memory jumpmap = new uint[](program.length);
      // So I'm about to use the output to store the temporary data I'm using to generate the
      // jumpmap. This could break with the provided spec, because the program can be any length,
      // which means we could get to a depth larger than 1024.
      // Fill in the jumpmap
      uint depth;
      for (uint j=0; j<program.length; j++){
        if (program[j]=="["){
          ...
          jumpmap[j] = output[depth];
          jumpmap[output[depth]] = j;
          depth -=1;
        } else if (program[j] == ">"){
          i = 1;
          while (program[j+i] == ">"){
