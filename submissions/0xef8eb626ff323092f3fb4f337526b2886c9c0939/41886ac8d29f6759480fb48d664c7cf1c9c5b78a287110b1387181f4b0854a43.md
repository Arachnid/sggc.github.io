...
challenge: Sort
gas: 5733005
submitted: 2018-06-07 18:19:14.143560+00:00
---
```solidity
/**
...
contract Sort {
    ...
    function quickSort(uint[] arr, uint low, uint high) private pure returns (uint[]) {
        if (low < high) {
            uint pi;
            (arr, pi) = partition2(arr, low, high);
    ...
    function partition2(uint[] arr, uint low, uint high) private pure returns (uint[], uint) {
        uint pivot = arr[high];
        uint i = low - 1;
